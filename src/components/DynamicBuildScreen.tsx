...
import { Search, CheckCircle, Assignment, Timeline, Build, Done } from '@mui/icons-material';
...
export default function DynamicBuildScreen({ progress, currentPhase, totalPhases }: DynamicBuildScreenProps) {
  const steps = [
    {
  ...
  const getPhaseProgress = () => {
    ...
    return (
      <Box sx={{ mt: 2 }}>
        ...
        <LinearProgress variant="determinate" value={(currentPhase / totalPhases) * 100} sx={{ height: 8, borderRadius: 4 }} />
      </Box>
    );
  };
      ...
      <MainCard>
        <CardContent>
          <Box sx={{ textAlign: 'center', mb: 4 }}>
            <CircularProgress variant="determinate" value={getProgressPercentage()} size={120} thickness={4} sx={{ mb: 2 }} />
            ...
            {steps.map((step, index) => {
              ...
              return (
                <Step key={step.type} active={isActive} completed={isCompleted}>
                  <StepLabel icon={isCompleted ? <CheckCircle color="success" /> : isActive ? <CircularProgress size={20} /> : step.icon}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      <Typography variant="subtitle1">{step.label}</Typography>
                      {isActive && <Chip label="In Progress" size="small" color="primary" variant="outlined" />}
                    </Box>
                  </StepLabel>
                  <StepContent>
